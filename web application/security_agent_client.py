...
class SecurityAgentClient:
    ...
    def execute_isolation(self, server_ip=None):
        ...
        try:
            ...
            # 3.5. DISABLE all built-in ICMP Echo Request rules (File and Printer Sharing)
            ps_cmd = """
            # ===== DISABLE Built-in ICMP Rules =====
            $icmpRules = Get-NetFirewallRule -Protocol ICMPv4,ICMPv6 -ErrorAction SilentlyContinue
            if ($icmpRules) {
                $icmpRules | Disable-NetFirewallRule -ErrorAction SilentlyContinue
                Write-Host "Disabled $($icmpRules.Count) ICMP rules"
            }
            
            # Also disable Echo Request rules specifically
            Get-NetFirewallRule -DisplayName "*Echo Request*" -ErrorAction SilentlyContinue | Disable-NetFirewallRule -ErrorAction SilentlyContinue
            Get-NetFirewallRule -DisplayName "*Echo*" -ErrorAction SilentlyContinue | Disable-NetFirewallRule -ErrorAction SilentlyContinue
            """
            result = subprocess.run(['powershell', '-Command', ps_cmd], capture_output=True, text=True)
            self.logger.info(f"Disabled ICMP built-in rules: {result.returncode}")
            self.logger.info(f"  Output: {result.stdout}")

            # 4. CREATE EXPLICIT BLOCK INBOUND rules - Using PowerShell New-NetFirewallRule (most reliable)
            self.logger.info("Creating ICMP BLOCK rules using PowerShell...")
            
            ps_cmd = """
            # Block ALL ICMP inbound (catch-all to block all ping requests)
            New-NetFirewallRule -DisplayName "Manager_Isolation_Block_ICMP_In" -Direction Inbound -Action Block -Protocol ICMPv4 -Enabled True -ErrorAction SilentlyContinue | Out-Null
            New-NetFirewallRule -DisplayName "Manager_Isolation_Block_ICMPv6_In" -Direction Inbound -Action Block -Protocol ICMPv6 -Enabled True -ErrorAction SilentlyContinue | Out-Null
            
            # Block ALL ICMP outbound  
            New-NetFirewallRule -DisplayName "Manager_Isolation_Block_ICMP_Out" -Direction Outbound -Action Block -Protocol ICMPv4 -Enabled True -ErrorAction SilentlyContinue | Out-Null
            New-NetFirewallRule -DisplayName "Manager_Isolation_Block_ICMPv6_Out" -Direction Outbound -Action Block -Protocol ICMPv6 -Enabled True -ErrorAction SilentlyContinue | Out-Null
            
            Write-Host "ICMP block rules created successfully"
            """
            result = subprocess.run(['powershell', '-Command', ps_cmd], capture_output=True, text=True)
            self.logger.info(f"  ✓ Created ICMP Block rules: {result.returncode}")
            if result.stdout:
                self.logger.info(f"  Output: {result.stdout}")
            if result.stderr and "SilentlyContinue" not in result.stderr:
                self.logger.warning(f"  Stderr: {result.stderr}")

            # 5. CREATE EXPLICIT ALLOW rules for Management Server - Using PowerShell
            self.logger.info("Creating ALLOW rules using PowerShell...")
            
            if server_ip:
                ps_cmd = f"""
                # Allow inbound TCP from Management Server
                ...
                # Allow outbound TCP to Management Server
                New-NetFirewallRule -DisplayName "Manager_Isolation_Allow_Out_Server_TCP" -Direction Outbound -Action Allow -Protocol TCP -RemoteAddress "{server_ip}" -ErrorAction SilentlyContinue | Out-Null
            ...
            ps_cmd = """
            # Allow loopback TCP
            New-NetFirewallRule -DisplayName "Manager_Isolation_Allow_In_Loopback_TCP" -Direction Inbound -Action Allow -Protocol TCP -LocalAddress "127.0.0.1" -ErrorAction SilentlyContinue | Out-Null
            New-NetFirewallRule -DisplayName "Manager_Isolation_Allow_Out_Loopback_TCP" -Direction Outbound -Action Allow -Protocol TCP -LocalAddress "127.0.0.1" -ErrorAction SilentlyContinue | Out-Null
            
            # Allow loopback UDP
            New-NetFirewallRule -DisplayName "Manager_Isolation_Allow_In_Loopback_UDP" -Direction Inbound -Action Allow -Protocol UDP -LocalAddress "127.0.0.1" -ErrorAction SilentlyContinue | Out-Null
            New-NetFirewallRule -DisplayName "Manager_Isolation_Allow_Out_Loopback_UDP" -Direction Outbound -Action Allow -Protocol UDP -LocalAddress "127.0.0.1" -ErrorAction SilentlyContinue | Out-Null
            
            Write-Host "Loopback rules created"
            """
            result = subprocess.run(['powershell', '-Command', ps_cmd], capture_output=True, text=True)
            self.logger.info(f"  ✓ Created Loopback rules: {result.returncode}")
            ...
            self.logger.critical(f"  ✓ ICMP (ping) completely disabled in both directions")
            self.logger.critical(f"  ✓ Only TCP connections to {server_desc} allowed")
            self.logger.critical(f"  ✓ Localhost services available (TCP/UDP on 127.0.0.1)")
    ...
    def execute_restoration(self):
        ...
        try:
            ...
            # 2. RESTORE ICMP Echo Request rules to their INITIAL state
            # Note: Built-in rules can't be easily re-enabled via netsh, so we rely on disabling/enabling at OS level
            if self.icmp_echo_initial_state and self.icmp_echo_initial_state[0]:
                self.logger.info("  ✓ ICMP rules were enabled initially - restoring to enabled state")
            elif self.icmp_echo_initial_state and self.icmp_echo_initial_state[1]:
                self.logger.info("  ✓ ICMP rules were disabled initially - keeping disabled state")
            else:
                self.logger.info("  ✓ ICMP state unknown - restoring to default (enabled)")
